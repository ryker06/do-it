...
export default function NotesPage() {
  ...
  function handleAdd() {
    ...
    // navigate to the newest note — it gets id note-${now}
    // slight delay to let store update
    setTimeout(() => {
      const id = `note-${now}`;
  ...
  function openNote(n: Note) {
    router.push(`/notes/detail?id=${n.id}`);
  }
      ...
      >
        ...
        {pinned.length > 0 && (
          <>
            ...
            {pinned.map((n) => (
              <NoteCard key={n.id} note={n} onTap={() => openNote(n)} />
            ))}
            {rest.length > 0 && (
              <div
                style={{
